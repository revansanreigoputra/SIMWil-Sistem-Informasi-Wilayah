...
<div class="card shadow-sm">
    <div class="card-header bg-primary text-white">
        <h5 class="mb-0">
            <i class="bi bi-people-fill me-2"></i>Form Tambah Data Lembaga Kemasyarakatan
        </h5>
    </div>

    <div class="card-body p-4">
        <form action="{{ route('potensi.potensi-kelembagaan.lembaga-kemasyarakatan.store') }}" method="POST">
            @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="jenis_lembaga_id" class="form-select" required>
                            ...
                            @foreach ($jenisLembaga as $jenis)
                                <option value="{{ $jenis->id }}" {{ old('jenis_lembaga_id') == $jenis->id ? 'selected' : '' }}>
                                    {{ $jenis->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="dasar_hukum_id" class="form-select" required>
                            ...
                            @foreach ($dasarHukum as $dasar)
                                <option value="{{ $dasar->id }}" {{ old('dasar_hukum_id') == $dasar->id ? 'selected' : '' }}>
                                    {{ $dasar->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="ruang_lingkup_kegiatan" class="form-control"
                            placeholder="Contoh: Desa / Kecamatan" value="{{ old('ruang_lingkup_kegiatan') }}">
                    </div>
                </div>
            </div>
...
<div class="alert alert-danger mt-3">
    ...
    Kamu tidak memiliki izin untuk menambahkan data Lembaga Kemasyarakatan.
</div>
@endcan
@endsection
