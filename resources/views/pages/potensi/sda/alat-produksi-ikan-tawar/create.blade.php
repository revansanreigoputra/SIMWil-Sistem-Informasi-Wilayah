...
@section('title', 'Tambah Data Alat Produksi Ikan Tawar')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Alat Produksi Ikan Tawar
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('p-alat-produksi-ikan-tawar.store') }}" method="POST">
                @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('alat_produksi_ikan_tawar_id') is-invalid @enderror" id="alat_produksi_ikan_tawar_id"
                                ...
                                @foreach ($alatProduksiIkanTawars as $alat)
                                    <option value="{{ $alat->id }}" {{ old('alat_produksi_ikan_tawar_id') == $alat->id ? 'selected' : '' }}>
                                        {{ $alat->nama }}
                                    </option>
                                @endforeach
                            </select>
