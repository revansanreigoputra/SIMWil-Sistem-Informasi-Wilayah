...
@section('title', 'Tambah Data Hasil Produksi Hutan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Hasil Produksi Hutan
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('hasilhutan.store') }}" method="POST">
                @csrf
                            ...
                            <select class="form-select @error('komoditas_hutan_id') is-invalid @enderror"
                                ...
                                @foreach ($komoditasHutans as $komoditas)
                                    <option value="{{ $komoditas->id }}"
                                        {{ old('komoditas_hutan_id') == $komoditas->id ? 'selected' : '' }}>
                                        {{ $komoditas->nama }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('satuan_produksi_kehutanan_id') is-invalid @enderror"
                                ...
                                @foreach ($satuans as $satuan)
                                    <option value="{{ $satuan->id }}"
                                        {{ old('satuan_produksi_kehutanan_id') == $satuan->id ? 'selected' : '' }}>
                                        {{ $satuan->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="dijual_ke_lumbung_desa_kelurahan">
                                <option value="Tidak" {{ old('dijual_ke_lumbung_desa_kelurahan') == 'Tidak' ? 'selected' : '' }}>
                                    Tidak</option>
                                <option value="Ya" {{ old('dijual_ke_lumbung_desa_kelurahan') == 'Ya' ? 'selected' : '' }}>Ya
                                </option>
                            </select>
                        </div>
                    </div>
