...
@section('title', 'Tambah Data Kondisi Hutan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Kondisi Hutan
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('kondisihutan.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('jenis_hutan_id') is-invalid @enderror"
                                ...
                                <option value="" disabled selected>Pilih Jenis Hutan</option>
                                @foreach ($jenisHutans as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ old('jenis_hutan_id') == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('kondisi_baik') is-invalid @enderror" id="kondisi_baik"
                                name="kondisi_baik" value="{{ old('kondisi_baik', 0) }}">
                            @error('kondisi_baik')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('kondisi_buruk') is-invalid @enderror" id="kondisi_buruk"
                                name="kondisi_buruk" value="{{ old('kondisi_buruk', 0) }}">
                            @error('kondisi_buruk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
