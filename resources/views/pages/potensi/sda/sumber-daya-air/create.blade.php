...
@section('title', 'Tambah Data Sumber Daya Air')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Sumber Daya Air
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('sumber-daya-air.store') }}" method="POST">
                @csrf
                            ...
                            <select class="form-select @error('jenis_potensi_air_id') is-invalid @enderror"
                                ...
                                @foreach ($jenisPotensiAirs as $jenisAir)
                                    <option value="{{ $jenisAir->id }}"
                                        {{ old('jenis_potensi_air_id') == $jenisAir->id ? 'selected' : '' }}>
                                        {{ $jenisAir->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="1"
                                ...
                                name="jumlah_luas" value="{{ old('jumlah_luas', 0) }}" required>
                            @error('jumlah_luas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('debit_volume') is-invalid @enderror" id="debit_volume"
                                ...
                                @foreach (['kecil', 'sedang', 'besar'] as $debit)
                                    <option value="{{ $debit }}"
                                        {{ old('debit_volume') == $debit ? 'selected' : '' }}>
                                        {{ ucfirst($debit) }}
                                    </option>
                                @endforeach
                            </select>
