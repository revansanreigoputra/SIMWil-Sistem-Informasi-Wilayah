...
@section('title', 'Tambah Data Dampak Pengolahan Hutan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Dampak Pengolahan Hutan
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('dampakpengolahan.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('jenis_dampak_id') is-invalid @enderror"
                                ...
                                <option value="" disabled selected>Pilih Jenis Dampak</option>
                                @foreach ($jenisDampaks as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ old('jenis_dampak_id') == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('keterangan') is-invalid @enderror" id="keterangan"
                                ...
                                <option value="Ada" {{ old('keterangan') == 'Ada' ? 'selected' : '' }}>Ada</option>
                                <option value="Tidak Ada" {{ old('keterangan') == 'Tidak Ada' ? 'selected' : '' }}>Tidak Ada</option>
                            </select>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
