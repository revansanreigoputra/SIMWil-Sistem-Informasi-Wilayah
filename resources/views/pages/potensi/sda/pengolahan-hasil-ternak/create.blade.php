...
@section('title', 'Tambah Data Pengolahan Hasil Ternak')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Pengolahan Hasil Ternak
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('pengolahan-hasil-ternak.store') }}" method="POST">
                @csrf
                            ...
                            <select class="form-select @error('jenis_olahan_hasil_ternak_id') is-invalid @enderror"
                                ...
                                @foreach ($jenisUsahaPengolahanHasilTernaks as $jenisUsaha)
                                    <option value="{{ $jenisUsaha->id }}"
                                        {{ old('jenis_olahan_hasil_ternak_id') == $jenisUsaha->id ? 'selected' : '' }}>
                                        {{ $jenisUsaha->nama ?? '' }}
                                    </option>
                                @endforeach
                            </select>
