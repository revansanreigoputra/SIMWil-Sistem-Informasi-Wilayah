...
@section('title', 'Tambah Potensi Cacat')

@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="card-title">
                 Tambah Data Potensi Cacat
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('potensi.potensi-sdm.cacat.store') }}" method="POST" id="form-p-cacat">
                @csrf
                            ...
                            <select name="cacat_id" id="cacat_id"
                                ...
                                @foreach ($cacats as $cacat)
                                    <option value="{{ $cacat->id }}"
                                        {{ old('cacat_id') == $cacat->id ? 'selected' : '' }}>
                                        {{ $cacat->nama_cacat }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="form-group d-flex justify-content-end">
                            <a href="{{ route('potensi.potensi-sdm.cacat.index') }}" class="btn btn-secondary">
...
@push('addon-script')
    <script>
        $(document).ready(function() {
            ...
            // Initial calculation if values are pre-filled (e.g., on validation error)
            calculateTotal();

            // Form validation
            $('#form-p-cacat').on('submit', function(e) {
                let isValid = true;
