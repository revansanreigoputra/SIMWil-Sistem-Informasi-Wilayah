...
@section('title', 'Tambah Potensi Agama')

@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="card-title">
                 Tambah Data Potensi Agama
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('potensi.potensi-sdm.agama.store') }}" method="POST" id="form-p-agama">
                @csrf
                            ...
                            <select name="id_agama" id="id_agama"
                                ...
                                @foreach ($agamas as $agama)
                                    <option value="{{ $agama->id }}"
                                        {{ old('id_agama') == $agama->id ? 'selected' : '' }}>
                                        {{ $agama->agama }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="form-group d-flex justify-content-end">
                            <a href="{{ route('potensi.potensi-sdm.agama.index') }}" class="btn btn-secondary">
...
@push('addon-script')
    <script>
        $(document).ready(function() {
            ...
            // Form validation
            $('#form-p-agama').on('submit', function(e) {
                let isValid = true;
