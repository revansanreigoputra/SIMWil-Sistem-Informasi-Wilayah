...
@section('title', 'Tambah Data Komunikasi Informasi')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Komunikasi Informasi
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('potensi.potensi-prasarana-dan-sarana.komunikasiinformasi.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('satuan') is-invalid @enderror" id="satuan"
                                name="satuan" value="{{ old('satuan') }}" placeholder="Contoh: unit, buah, dll" required>
                            @error('satuan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
@push('addon-script')
    <script>
        $(document).ready(function() {
            ...
            // Jika ada old value (validasi gagal), isi dropdown jenis sesuai kategori lama
            @if (old('kategori_id'))
                loadJenis("{{ old('kategori_id') }}", "{{ old('jenis_id') }}");
            @endif
        });
    </script>
@endpush
