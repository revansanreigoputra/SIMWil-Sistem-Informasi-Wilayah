...
@section('title', 'Tambah Data Prasarana Pendidikan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Prasarana Pendidikan
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('potensi.potensi-prasarana-dan-sarana.ppendidikan.store') }}" method="POST">
                @csrf
                            ...
                            <select class="form-select @error('jenis_gedung_id') is-invalid @enderror"
                                ...
                                @foreach ($jenisGedungs as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ old('jenis_gedung_id') == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('jumlah_sewa') is-invalid @enderror"
                                id="jumlah_sewa" name="jumlah_sewa" value="{{ old('jumlah_sewa', 0) }}" min="0" required>
                            @error('jumlah_sewa')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                     ...
                     <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('jumlah_milik_sendiri') is-invalid @enderror"
                                id="jumlah_milik_sendiri" name="jumlah_milik_sendiri" value="{{ old('jumlah_milik_sendiri', 0) }}" min="0" required>
