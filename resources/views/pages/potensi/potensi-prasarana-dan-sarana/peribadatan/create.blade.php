...
@section('title', 'Tambah Data Prasarana Peribadatan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Tambah Data Prasarana Peribadatan
            </h5>
        </div>

        <div class="card-body">
            <form action="{{ route('potensi.potensi-prasarana-dan-sarana.peribadatan.store') }}" method="POST">
                @csrf
                            ...
                            <select class="form-select @error('tempat_ibadah_id') is-invalid @enderror"
                                ...
                                @foreach ($tempatIbadahs as $tempat)
                                    <option value="{{ $tempat->id }}"
                                        {{ old('tempat_ibadah_id') == $tempat->id ? 'selected' : '' }}>
                                        {{ $tempat->nama_tempat }}
                                    </option>
                                @endforeach
                            </select>
