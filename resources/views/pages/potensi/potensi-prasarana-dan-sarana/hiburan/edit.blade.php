...
@section('title', 'Edit Data Prasarana Hiburan')
    ...
    <div class="card shadow-sm">
        <div class="card-header text-dark">
            <h5 class="card-title mb-0">
                ...
                Edit Data Prasarana Hiburan
            </h5>
        </div>

        <div class="card-body">
            <form
                action="{{ route('potensi.potensi-prasarana-dan-sarana.hiburan.update', ['prasaranahiburan' => $prasaranahiburan->id]) }}"
                method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal') is-invalid @enderror"
                                ...
                                value="{{ old('tanggal', $prasaranahiburan->tanggal?->format('Y-m-d')) }}" required>
                            @error('tanggal')
                            ...
                            <select class="form-select @error('jphiburan_id') is-invalid @enderror"
                                ...
                                @foreach ($jphiburans as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ old('jphiburan_id', $prasaranahiburan->jphiburan_id) == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('jumlah') is-invalid @enderror"
                                ...
                                value="{{ old('jumlah', $prasaranahiburan->jumlah) }}" min="0" required>
                            @error('jumlah')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
