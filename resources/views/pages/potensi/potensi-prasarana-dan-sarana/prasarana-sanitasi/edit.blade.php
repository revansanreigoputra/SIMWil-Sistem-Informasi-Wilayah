...
@section('title', 'Edit Data Prasarana Sanitasi')
...
<div class="card shadow-sm">
    <div class="card-header text-dark">
        <h5 class="card-title mb-0">
            ...
            Edit Data Prasarana Sanitasi
        </h5>
    </div>

    <div class="card-body">
        <form action="{{ route('potensi.potensi-prasarana-dan-sarana.prasarana-sanitasi.update', $sanitasi->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('desa_id') is-invalid @enderror" id="desa_id" name="desa_id" required>
                            ...
                            @foreach($desas as $desa)
                                <option value="{{ $desa->id }}" {{ old('desa_id', $sanitasi->desa_id) == $desa->id ? 'selected' : '' }}>
                                    {{ $desa->nama_desa }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal') is-invalid @enderror"
                            id="tanggal" name="tanggal" value="{{ old('tanggal', $sanitasi->tanggal->format('Y-m-d')) }}" required>
                        @error('tanggal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('sumur_resapan_air') is-invalid @enderror"
                            id="sumur_resapan_air" name="sumur_resapan_air" value="{{ old('sumur_resapan_air', $sanitasi->sumur_resapan_air) }}"
                            min="0" required>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('mck_umum') is-invalid @enderror"
                            id="mck_umum" name="mck_umum" value="{{ old('mck_umum', $sanitasi->mck_umum) }}"
                            min="0" required>
                        @error('mck_umum')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('jamban_keluarga') is-invalid @enderror"
                            id="jamban_keluarga" name="jamban_keluarga" value="{{ old('jamban_keluarga', $sanitasi->jamban_keluarga) }}"
                            min="0" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('saluran_drainase') is-invalid @enderror"
                            ...
                            @foreach($saluranDrainaseOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('saluran_drainase', $sanitasi->saluran_drainase) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kondisi_saluran') is-invalid @enderror"
                            ...
                            @foreach($kondisiSaluranOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('kondisi_saluran', $sanitasi->kondisi_saluran) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
