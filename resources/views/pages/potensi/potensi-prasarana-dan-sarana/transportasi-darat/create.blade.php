    ...
    <div class="card-body">
        <form action="{{ route('potensi.potensi-prasarana-dan-sarana.transportasi-darat.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="jenis_sarana_prasarana" name="jenis_sarana_prasarana" required>
                    ...
                    @foreach($jenisSaranaPrasaranaOptions as $key => $value)
                        <option value="{{ $key }}" {{ old('jenis_sarana_prasarana') == $key ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="kondisi_baik" name="kondisi_baik" value="{{ old('kondisi_baik') }}" min="0" required placeholder="Contoh: 100">
                @error('kondisi_baik')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="kondisi_rusak" name="kondisi_rusak" value="{{ old('kondisi_rusak') }}" min="0" required placeholder="Contoh: 200">
                @error('kondisi_rusak')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
