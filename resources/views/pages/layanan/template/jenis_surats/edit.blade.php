        ...
        <div class="card shadow-sm border-0 rounded-3">
            ...
            <div class="card-body bg-light">
                ...
                <form action="{{ route('jenis_surats.update', $jenisSurat->id) }}" method="POST">
                    ...
                    <div id="initial-variables-data"
                        data-variables="{{ json_encode($jenisSurat->required_variables ?? []) }}" style="display: none;">
                    </div>
                    <div id="system-variables-map" data-map="{{ json_encode($systemVariables) }}" style="display: none;">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kode') is-invalid @enderror" id="kode"
                            name="kode" value="{{ old('kode', $jenisSurat->kode) }}" placeholder="Format: /SKU/ ">
                        @error('kode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="nama" class="form-label fw-semibold text-dark">Nama Jenis Dokumen <span
                                class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama"
                            name="nama" value="{{ old('nama', $jenisSurat->nama) }}"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="mutasi_type" id="mutasi_type"
                                    ...
                                    <option value="none"
                                        {{ old('mutasi_type', $jenisSurat->mutasi_type) == 'none' ? 'selected' : '' }}>
                                        0. Tidak Ada Mutasi (Default)
                                    </option>
                                    <option value="meninggal"
                                        {{ old('mutasi_type', $jenisSurat->mutasi_type) == 'meninggal' ? 'selected' : '' }}>
                                        01. Mutasi Kematian (Status Menjadi Meninggal)
                                    </option>
                                    <option value="pindah_keluar"
                                        {{ old('mutasi_type', $jenisSurat->mutasi_type) == 'pindah_keluar' ? 'selected' : '' }}>
                            ...
                            <small class="text-muted">Pilih data penduduk/anggota keluarga yang diperlukan sesuai jenis
                                surat ini atau tambah inputan baru.</small>
                    ...
                    <div class="mb-3 mt-3">
                        ...
                        <textarea name="paragraf_pembuka" rows="3" id="paragraf_pembuka"
                            class="form-control @error('paragraf_pembuka') is-invalid @enderror" placeholder="Ini paragraph pembuka disurat">{{ old('paragraf_pembuka', $jenisSurat->paragraf_pembuka) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="paragraf_penutup" rows="3" id="paragraf_penutup"
                            class="form-control @error('paragraf_penutup') is-invalid @enderror" placeholder="Ini paragraph penutup disurat">{{ old('paragraf_penutup', $jenisSurat->paragraf_penutup) }}</textarea>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <a href="{{ route('jenis_surats.index') }}" class="btn btn-outline-primary rounded-pill px-4 me-2">
...
@push('addon-script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const initialVariables = JSON.parse(document.getElementById('initial-variables-data').dataset
                .variables);
