            ...
            <div class="card-body bg-light">
                <form action="{{ route('jenis_surats.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="kop_template_id" id="kop_template_id"
                            class="form-select
                        @error('kop_template_id') is-invalid @enderror"
                            required>
                    ...
                    <div class="mb-3">
                        <label for="kode" class="form-label fw-semibold text-dark">Kode Surat / Laporan <span
                        ...
                        <input type="text" class="form-control @error('kode') is-invalid @enderror" id="kode"
                            name="kode" value="{{ old('kode') }}" placeholder="Format: /SKU/ ">
                        @error('kode')
                    ...
                    <div class="mb-3">
                        <label for="nama" class="form-label fw-semibold text-dark">Nama Jenis Laporan/Surat <span
                                class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama"
                            name="nama" value="{{ old('nama') }}" placeholder="Contoh: SURAT KETERANGAN USAHA">
                        @error('nama')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="card bg-card-inside-body p-3">
                        <div class="row mb-3">
                            <h5 class="mt-4 mb-3 pb-2 border-bottom fw-bold">Pengaturan Mutasi Data Kependudukan</h5>

                            <div class="mb-3">
                                <label for="mutasi_type" class="form-label fw-semibold text-dark">Efek Mutasi Data</label>
                                <small class="text-muted d-block mb-1">Pilih jenis mutasi yang akan dipicu oleh surat
                                    ini.</small>
                                <select name="mutasi_type" id="mutasi_type"
                                    class="form-select @error('mutasi_type') is-invalid @enderror" required>
                                    <option value="none" {{ old('mutasi_type') == 'none' ? 'selected' : '' }}>
                                        0. Tidak Ada Mutasi (Default)
                                    </option>
                                    <option value="meninggal" {{ old('mutasi_type') == 'meninggal' ? 'selected' : '' }}>
                                        01. Mutasi Kematian (Mengubah Status Anggota Keluarga)
                                    </option>
                                    <option value="pindah_keluar"
                                        {{ old('mutasi_type') == 'pindah_keluar' ? 'selected' : '' }}>
                                        02. Mutasi Pindah Keluar (Mengurangi Data)
                                    </option>
                                    <option value="mutasi_masuk_kk"
                                        {{ old('mutasi_type') == 'mutasi_masuk_kk' ? 'selected' : '' }}>
                            ...
                            {{-- Lanjutkan dengan bagian "add or retrieve anggota keluarga start" Anda di bawah ini. --}}
                            <div class="col-md-4">
                                <label class="form-label">Data Penduduk</label>
                            </div>
                            <div class="col-md-8 d-flex justify-content-end">
                                <button type="button" id="add-variable-btn" class="btn btn-sm btn-primary p-2 mt-2">Tambah
                                    Data Inputan</button>
                            </div>
                            <small class="text-muted">Isi data penduduk yang diperlukan sesuai jenis surat/laporan ini.
                            </small>
                        ...
                        <template id="variable-row-template">
                            <div class="input-group mb-3 variable-row">
                                ...
                                <input type="hidden" name="variables[TEMP_INDEX][type]" value="system"
                                    class="variable-type-hidden">

                                <div class="custom-key-container d-none ms-2">
                                    {{-- CUSTOM TEXT INPUTS --}}
                                    <input type="text" name="variables[TEMP_INDEX][key]"
                                        placeholder="Kata Kunci Data (tgl_kematian)"
                                        class="form-control variable-key-input">
                                    <input type="text" name="variables[TEMP_INDEX][label]"
                                        placeholder="Label/Nama Tampilan" class="form-control variable-label-input">

                                    {{-- CUSTOM TYPE SELECT: OVERRIDES HIDDEN INPUT --}}
                                    <select name="variables[TEMP_INDEX][type]" class="form-select variable-type-select ms-1"
                                        style="width: 150px;">
                    ...
                    <div class="mb-3">
                        <label for="paragraf_pembuka" class="form-label fw-semibold text-dark">Paragraf Pembuka <span
                                class="text-danger">*</span></label>

                        <textarea name="paragraf_pembuka" rows="3" id="paragraf_pembuka"
                            class="form-control @error('paragraf_pembuka') is-invalid @enderror" placeholder="Ini paragraph pembuka  ">{{ old('paragraf_pembuka') }}</textarea>
                    ...
                    <div class="mb-3">
                        <label for="paragraf_penutup" class="form-label fw-semibold text-dark">Paragraf Penutup <span
                                class="text-danger">*</span></label>

                        <textarea name="paragraf_penutup" rows="3" id="paragraf_penutup"
                            class="form-control @error('paragraf_penutup') is-invalid @enderror" placeholder="Ini paragraf penutup  ">{{ old('paragraf_penutup') }}</textarea>
            ...
            variableList.addEventListener('change', function(e) {
                ...
                if (e.target.classList.contains('variable-source-select')) {
                    ...
                    if (e.target.value === '__CUSTOM__') {
                        ...
                        labelInput.value = e.target.options[e.target.selectedIndex]
                        .text; // Set the display label
                    }
                }
            });
        });
