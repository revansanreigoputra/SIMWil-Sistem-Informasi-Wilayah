...
@section('content') 
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            ...
            {{-- 2. INFORMASI KARTU KELUARGA --}}
            <h4 class="mt-4 mb-3 pb-2 border-bottom">Data Kartu Keluarga</h4>
            @php
            ...
            @if ($kk)
                <dl class="row">
                    ...
                    <dd class="col-sm-9">{{ $kk->alamat }}, RT {{ $kk->rt }}/RW {{ $kk->rw }}</dd>
                    
                    <dt class="col-sm-3">Wilayah:</dt>
                    {{-- Asumsi relasi Desa/Kecamatan dimuat melalui DataKeluarga --}}
                    <dd class="col-sm-9">{{ optional($kk->desas)->nama_desa ?? 'N/A' }}, {{ optional($kk->kecamatans)->nama_kecamatan ?? 'N/A' }}</dd>
                </dl>
            @else
            ...
            <div class="alert alert-{{ $record->status_kehidupan === 'meninggal' ? 'danger' : 'warning' }}">
                <dl class="row">
                    ...
                    <dd class="col-sm-9 fw-bold text-uppercase">{{ Str::title(str_replace('_', ' ', $record->status_kehidupan)) }}</dd>
