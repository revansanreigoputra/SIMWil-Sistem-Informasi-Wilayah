        ...
        <form action="{{ route('perkembangan.pendapatanperkapital.pendapatan_rill_keluarga.update', $item->id) }}" method="POST">
            ...
            <div class="row mt-2">
                <div class="col-md-4 mb-2">
                    ...
                    <input type="number" name="kk" id="kk" class="form-control" value="{{ old('kk', $item->kk) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Jumlah Anggota Keluarga (AK)</label>
                    <input type="number" name="ak" id="ak" class="form-control" value="{{ old('ak', $item->ak) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Pendapatan Kepala Keluarga (Rp)</label>
                    <input type="number" name="pendapatan_kk" id="pendapatan_kk" class="form-control" value="{{ old('pendapatan_kk', $item->pendapatan_kk) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Pendapatan Anggota Keluarga (Rp)</label>
                    <input type="number" name="pendapatan_ak" id="pendapatan_ak" class="form-control" value="{{ old('pendapatan_ak', $item->pendapatan_ak) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Total Pendapatan (Rp)</label>
                    <input type="number" id="total1" name="total1" class="form-control" value="{{ old('total1', $item->total1) }}" readonly>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Pendapatan per Anggota (Rp)</label>
                    <input type="number" id="total2" name="total2" class="form-control" value="{{ old('total2', $item->total2) }}" readonly>
                </div>
            </div>
