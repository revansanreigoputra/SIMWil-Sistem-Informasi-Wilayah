        ...
        <form action="{{ route('perkembangan.pendidikanmasyarakat.wajib_belajar_9_tahun.update', $item->id) }}" method="POST">
            ...
            <div class="row mt-2">
                <div class="col-md-4 mb-2">
                    ...
                    <input type="number" name="penduduk" id="penduduk" class="form-control" 
                        value="{{ old('penduduk', $item->penduduk) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Jumlah Masih Sekolah (Orang)</label>
                    <input type="number" name="masih_sekolah" id="masih_sekolah" class="form-control" 
                        value="{{ old('masih_sekolah', $item->masih_sekolah) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Jumlah Tidak Sekolah (Orang)</label>
                    <input type="number" name="tidak_sekolah" id="tidak_sekolah" class="form-control" 
                        value="{{ old('tidak_sekolah', $item->tidak_sekolah) }}" required>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Persentase Masih Sekolah (%)</label>
                    <input type="number" id="persentase_masih" name="persentase_masih" class="form-control" 
                        value="{{ old('persentase_masih', $item->persentase_masih ?? '') }}" readonly>
                </div>
                <div class="col-md-4 mb-2">
                    <label>Persentase Tidak Sekolah (%)</label>
                    <input type="number" id="persentase_tidak" name="persentase_tidak" class="form-control" 
                        value="{{ old('persentase_tidak', $item->persentase_tidak ?? '') }}" readonly>
                </div>
            </div>
