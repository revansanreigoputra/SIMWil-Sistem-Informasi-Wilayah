...
@section('title', 'Edit Data Sikap dan Mental Kebangsaan')
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white border-bottom">
        <h5 class="card-title mb-0">
            ...
            Edit Data Sikap dan Mental Kebangsaan
        </h5>
    </div>

    <div class="card-body">
        <form action="{{ route('perkembangan.kedaulatanmasyarakat.berbangsa.update', $berbangsa->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror"
                           ...
                           value="{{ old('tanggal', \Carbon\Carbon::parse($berbangsa->tanggal)->format('Y-m-d')) }}" required>
                    @error('tanggal')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="id_desa" id="id_desa" class="form-control" required>
                        ...
                        @foreach ($desas as $desa)
                            <option value="{{ $desa->id }}" {{ old('id_desa', $berbangsa->id_desa) == $desa->id ? 'selected' : '' }}>
                                {{ $desa->nama_desa }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-3">
                        <input type="number" name="{{ $name }}" min="0"
                               ...
                               value="{{ old($name, $berbangsa->$name) }}" required>
                        @error($name)
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
