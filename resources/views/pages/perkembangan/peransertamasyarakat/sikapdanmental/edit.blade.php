...
@section('title', 'Edit Data Sikap dan Mental Masyarakat')
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white border-bottom">
        <h5 class="card-title mb-0">
            ...
            Edit Data Sikap dan Mental Masyarakat
        </h5>
    </div>

    <div class="card-body">
        <form action="{{ route('perkembangan.peransertamasyarakat.sikapdanmental.update', $sikapdanmental->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror"
                        id="tanggal" name="tanggal" value="{{ old('tanggal', $sikapdanmental->tanggal) }}" required>
                    @error('tanggal')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="id_desa" id="id_desa" class="form-control" required>
                        ...
                        @foreach ($desas as $item)
                            <option value="{{ $item->id }}" {{ old('id_desa', $sikapdanmental->id_desa) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_desa }}
                            </option>
                        @endforeach
                    </select>
                ...
                @foreach ($numberFields as $name => $label)
                    <div class="col-md-6 mb-3">
                        ...
                        <input 
                            ...
                            value="{{ old($name, $sikapdanmental->$name) }}"
                            required
                        >
                        @error($name)
                            <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        @foreach (['Ada', 'Tidak Ada'] as $option)
                            <div class="form-check form-check-inline">
                                <input 
                                    ...
                                    name="{{ $name }}" 
                                    value="{{ $option }}"
                                    {{ old($name, $sikapdanmental->$name) === $option ? 'checked' : '' }} 
                                    required
                                >
