        ...
        <form action="{{ route('perkembangan.pemerintahdesadankelurahan.pembinaankecamatan.update', $pembinaankecamatan->id) }}" method="POST">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" 
                    ...
                    value="{{ old('tanggal', \Carbon\Carbon::parse($pembinaankecamatan->tanggal)->format('Y-m-d')) }}" required>
                @error('tanggal')
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="id_desa" id="id_desa" class="form-control" required>
                    ...
                    @foreach ($desas as $item)
                        <option value="{{ $item->id }}" {{ old('id_desa', $pembinaankecamatan->id_desa) == $item->id ? 'selected' : '' }}>
                            {{ $item->nama_desa }}
                        </option>
                    @endforeach
                </select>
